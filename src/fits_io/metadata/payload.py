...
from fits_io.metadata.resolve import remap_channel_indices, resolve_channel_selection, resolve_output_axes
...
def build_payload(reader: ImageReader,
    ...
    Args:
        reader (ImageReader): The image reader containing the base metadata.
        channel_labels (Sequence[str] | None): The list of channel labels to use for the output artifact. If None, the labels from the reader will be used.
        export_channels (str, Sequence[str]): The channels to export. Can be 'all' or a list of channel labels.
        artifact_type (str | None): The type of artifact being processed (e.g., "raw_image", "segmentation"). If None, the artifact type from the reader's metadata will be used.
        created_by (str | None): The name of the software or tool that created the artifact. If None, the created_by from the reader's metadata will be used.
        z_projection (Zproj): The z-projection method to apply ('max', 'mean', or None).
        custom_metadata (Mapping[str, Any] | None): Additional custom metadata to include in the payload. If None, the custom metadata from the reader's metadata will be used.
        array_shape (tuple[int, ...] | None): The shape of the image array. If provided, it will be used to validate the channel selection against the expected output axes.
    ...
    selection = resolve_channel_selection(
                channel_labels,
                n_channels=reader.channel_count,
...
def assemble_payload(base: FitsIOMeta, 
    ...
    Args:
        base (FitsIOPayload): The base payload to build upon.
        ...
        artifact_type (str): The type of artifact being processed (e.g., "raw_image", "segmentation").
        created_by (str): The name of the software or tool that created the artifact.
        derived_from (str): The type of artifact from which this artifact was derived.
        source_channel_indices (Sequence[int]): The indices of the channels in the source image data.
        artifact_channel_indices (Sequence[int]): The indices of the channels in this artifact image data.
